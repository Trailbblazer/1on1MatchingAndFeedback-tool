    ...
    try:
        ...
        for member in members:
            ...
            if member.get("level") == "primary":
                has_primary = True
        if not has_primary: return jsonify(
            {"error": "At least one member must be marked as primary (level='primary')"}), 400
...
def get_startups():
    try:
        ...
        for startup in startups:
            # Return all columns dynamically so you can refine later
